...
void SQL_CUSTOM::getResult(Custom_Call_UnorderedMap::const_iterator &custom_calls_itr, Poco::Data::Session &session, Poco::Data::Statement &sql_statement, std::string &player_key, std::string &result, bool &status)
{
	try
	{
		result = "[1,[";
...
void SQL_CUSTOM::callPreparedStatement(std::string call_name, Custom_Call_UnorderedMap::const_iterator custom_calls_itr, std::vector< std::vector< std::string > > &all_processed_inputs, std::string &player_key, bool &status, std::string &result)
{
...
void SQL_CUSTOM::callPreparedStatement(std::string call_name, Custom_Call_UnorderedMap::const_iterator custom_calls_itr, std::vector< std::vector<std::string> > &all_processed_inputs, std::vector<std::string> &custom_inputs, std::string &player_key, bool &status, std::string &result)
{
	Poco::Data::SessionPool::SessionDataPtr session_data_ptr;
	try
	{
...
bool SQL_CUSTOM::callProtocol(std::string input_str, std::string &result, const bool async_method, const unsigned int unique_id)
{
	...
	else
	{
		...
		else
		{
			...
			if (!status)
			{
				extension_ptr->logger->warn("extDB2: SQL_CUSTOM: Error: UniqueID: {0} Input String: {1}", unique_id, input_str);
			}
		}
	}
	return true;

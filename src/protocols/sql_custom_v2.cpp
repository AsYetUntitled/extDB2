...
bool SQL_CUSTOM_V2::init(AbstractExt *extension, const std::string &database_id, const std::string init_str)
{
	...
	if (status)
	{
		...
		for (auto &custom_ini_file : custom_ini_files)
		{
			...
			#ifdef DEBUG_TESTING
				extension_ptr->console->info("extDB2: SQL_CUSTOM_V2: Loading Template Filename: {0}", db_template_file_str);
			#endif
			extension_ptr->logger->info("extDB2: SQL_CUSTOM_V2: Loading Template Filename: {0}", db_template_file_str);
			...
			else
			{
				status = false;
				#ifdef DEBUG_TESTING
					extension_ptr->console->warn("extDB2: SQL_CUSTOM_V2: Template File Not Found: {0}", db_template_file_str);
				#endif
				extension_ptr->logger->warn("extDB2: SQL_CUSTOM_V2: Template File Not Found: {0}", db_template_file_str);
				break;
			}
		}
	}
...
void SQL_CUSTOM_V2::callPreparedStatement(std::string call_name, Custom_Call_UnorderedMap::const_iterator custom_calls_itr, std::vector< std::vector< std::string > > &all_processed_inputs, bool &status, std::string &result)
{
...
void SQL_CUSTOM_V2::callPreparedStatement(std::string call_name, Custom_Call_UnorderedMap::const_iterator custom_calls_itr, std::vector< std::vector<std::string> > &all_processed_inputs, std::vector<std::string> custom_inputs, bool &status, std::string &result)
{
	Poco::Data::SessionPool::SessionDataPtr session_data_ptr;
	try
	{
...
bool SQL_CUSTOM_V2::callProtocol(std::string input_str, std::string &result, const int unique_id)
{
	...
	if (custom_calls_const_itr == custom_calls.end())
	{
		...
		extension_ptr->logger->warn("extDB2: SQL_CUSTOM_V2: Error No Custom Call Not Found: Callname {0}", tokens[0]);
		#ifdef DEBUG_TESTING
			extension_ptr->console->warn("extDB2: SQL_CUSTOM_V2: Error No Custom Call Not Found: Input String {0}", input_str);
			extension_ptr->console->warn("extDB2: SQL_CUSTOM_V2: Error No Custom Call Not Found: Callname {0}", tokens[0]);
		#endif
	}
	else
	{
		if ((custom_calls_const_itr->second.number_of_inputs + custom_calls_const_itr->second.number_of_custom_inputs) != (tokens.count() - 1))
		{
			...
			extension_ptr->logger->warn("extDB2: SQL_CUSTOM_V2: Incorrect Number of Inputs: Expected: {0} Got: {1}", (custom_calls_const_itr->second.number_of_inputs + custom_calls_const_itr->second.number_of_custom_inputs), (tokens.count() - 1));
			#ifdef DEBUG_TESTING
				extension_ptr->console->warn("extDB2: SQL_CUSTOM_V2: Incorrect Number of Inputs: Input String {0}", input_str);
				extension_ptr->console->warn("extDB2: SQL_CUSTOM_V2: Incorrect Number of Inputs: Expected: {0} Got: {1}", (custom_calls_const_itr->second.number_of_inputs + custom_calls_const_itr->second.number_of_custom_inputs), (tokens.count() - 1));
			#endif
		}
		else
		{
